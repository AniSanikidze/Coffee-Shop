...
import { getOrderAdmin, updateOrder } from "../../../actions/orderAction";
...
const UpdateOrder = ({ match }) => {
  ...
  useEffect(() => {
      dispatch(getOrderAdmin(orderId))
    //   if ()
    // if (product && product._id !== orderId) {
    //     dispatch(getProductInfo(orderId));
    //   } else
    if(!loading){
             if (product){
          console.log(product)
  ...
  const updateOrderSubmitHandler = (e) => {
    ...
    // });
    dispatch(updateOrder(orderId,price,roastLevel,singleOrigin,origin,desc,stock,bagSize,productName));
  };
  const roastLevels = [
    "Light",
    "Medium",
